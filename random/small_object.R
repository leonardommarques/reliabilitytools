...
# 
# # -- document to insert
# doc_to_insert = r.doc_to_insert
# 
# # -- convert strings to dates
# doc_to_insert['analysis_period'] = list(map(lambda x: datetime.datetime.strptime(x, '%Y-%m-%d'), doc_to_insert['analysis_period']))
# 
# # -- change data.frames to DataFrames, mutate the dates and then convert to dict.
# doc_to_insert['ff_table'] = pd.DataFrame(doc_to_insert['ff_table'])
# doc_to_insert['ff_table']['month'] = doc_to_insert['ff_table']['month'].map(lambda x: datetime.datetime.strptime(str(x), '%Y-%m-%d'))
# doc_to_insert['ff_table'] = doc_to_insert['ff_table'].to_dict(orient='records')
# 
# doc_to_insert['surv_predictions'] = pd.DataFrame(doc_to_insert['surv_predictions'])
# doc_to_insert['surv_predictions']['month'] = doc_to_insert['surv_predictions']['month'].map(lambda x: datetime.datetime.strptime(str(x), '%Y-%m-%d'))
# doc_to_insert['surv_predictions'] = doc_to_insert['surv_predictions'].to_dict(orient='records')
# 
# # -- Save in mongo -- #
# db.reli_report.insert(doc_to_insert)
# 
# 
# exit
# ~.~.~.~.~.~.~.~.~.~.~.~.~.~.~.
...
###
# baixar o relatorio do(s) banco(s) e reconstruir os dados (analysis$data)
# calcular o tempo necessário para isso.
### 
...
repl_python()
doc_db = db.reli_report.find_one({"alias" : "repor to convert"})
...
# Pois é, aparentemente as datas estão bugadas mesmo
# melhor evitar usar R para MongoDb

# analysis_db must be equal to doc_to_insert
...
# -- dates -- #
db_doc$analysis_period = db_doc$analysis_period %>%
  as.Date()

# -- ff_table and surv_prediction -- #
db_doc$ff_table = db_doc$ff_table %>%
  bind_rows() %>%
  mutate(month = as.Date(month))

db_doc$surv_predictions = db_doc$surv_predictions %>%
  bind_rows() %>%
  mutate(month = as.Date(month))

# --- encapsular em uma função e mudar a classe para c("s_analysis_collection", "list" )-- #
...
cowplot::plot_grid(
  plot_surv_analys_collection(db_doc)
  , plot_surv_analys_collection(doc_to_insert)
  , nrow = 2
  , labels = c('db_doc', 'doc_to_insert'))
...
aux = list.files('C:/Leonardo/Projetos R/my_R_packages/reliabilitytools/python/'
                 , full.names = TRUE)



aux_doc = doc_to_insert

aux_doc$alias = 'via funcao nova'
aux %>% map(source_python)


analysys_collection_r_to_mongo(
  analysis_collection = 'aux_doc'
  , db_name='volvo'
  , collection_name = 'reli_report'
)
...
query = list(alias = aux_doc$alias[[1]])
doc_lido = analysis_loaded_from_mongo = analysis_collection_mongo_to_r(
  query
  , db_name = 'volvo'
  , collection_name = 'reli_report'
)

doc_lido$analysis_period
